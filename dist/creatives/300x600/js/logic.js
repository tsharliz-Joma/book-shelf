...
function setupAnimation() {
	...
	{
		// phakathi
		tl.from("#dash,#background,.border,#underline,#p1", { duration: 0.5, y: 600, ease: 'power3.inOut'}, time + "+=0.0")
		tl.from('#copy-one-container,#logo-officeworks,#disclaimer,#underline', { duration: 0.5, opacity: 0, ease: 'power2.out'} , time + '+=0.5');
		...
		tl.to('#dash', { duration: 0.5, y: -13, ease: 'power2.out'}, time + "+=0.25")
		...
		tl.set("#logo-officeworks", { x: -74, y: 524 })
	}
	...
	// FRAME 2
	{
		...
		// phakathi
		tl.set("#price-component", {width: compWidth.mrec.hundreds }, time + "+=0.0");
		...
		tl.from("#colored_panel_2", { duration: 0.5, opacity: 0, ease: 'power2.out' }, time + "+=0.55");
	...
	//  FRAME 3 
	{
		...
		tl.set("#ctas", { x: -10 }, time + "+=0.5");
	}
	// FRAME 4 
	{
		tl.to('#dash', { duration: 0.5, y: 0, ease: 'power2.out'}, time + "+=0.5")
