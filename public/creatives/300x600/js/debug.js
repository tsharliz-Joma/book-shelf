...
function setupTimeline() {
  ...
  if (tl.duration() > 15) {
    timelineBackground.style.cssText =
      ...
      "px;height:" +
      5 +
      "px;background:#AAA;pointer-events: none;";
  } else {
    timelineBackground.style.cssText =
      "position:absolute;top:" +
      25 +
      "px;left:" +
      (tl.duration() * magic + 10) +
      "px;width:" +
      (15 * magic - tl.duration() * magic) +
      "px;height:" +
      5 +
      "px;background:#333;pointer-events: none;";
  }
  timelineBackground.setAttribute("id", "tl-bg");

  timelineTags = document.createElement("div");
  timelineTags.style.cssText =
    "position:absolute;top:" +
    35 +
    "px;left:10px;width:" +
    tl.duration() * magic +
  ...
  if (tl.duration() > 15) {
    timeline15sec.style.cssText =
      ...
      "px;height:" +
      5 +
      "px;background:#CCC;pointer-events: none;";
  } else {
    timeline15sec.style.cssText =
      "position:absolute;top:" +
      25 +
  ...
  timelineProgress.style.cssText =
    ...
    "px;height:" +
    5 +
    "px;background:#009900;pointer-events: none;";
  timelineProgress.setAttribute("id", "tl-progress");

  timelineClickTag = document.createElement("div");
  timelineClickTag.style.cssText =
    "position:absolute;top:" +
    25 +
    "px;right:10px;height:" +
    10 +
    "px;pointer-events: none;font-family:sans-serif;font-size:11px;color:#000;display:none";
  ...
  for (var i = 0; i <= duration; i++) {
    ...
    timelineLines.style.cssText =
      ...
      "px;width:1px;height:" +
      75 +
      "px;background:#CCC;border-left:1px solid #DDD";
    //timelineScrub.setAttribute("id", "tl-scrub");
  ...
  document.getElementById("tl-scrub").addEventListener(
    "mousedown",
    (e) => {
...
function setupLabels() {
  ...
  for (var i in tl.labels) {
    ...
    labelBox.style.cssText =
      "postition:absolute;width:30px;text-align: center;position: absolute;background-color:rgba(225,225,225,0);cursor: pointer;";

    labelBox.style.cssText +=
      "left: " + (tl.labels[i] * magic - 16) + "px; top: 0px;";
    labelBox.setAttribute("id", "label-box-" + i);

    labelLine.style.cssText =
      "position: relative;left: 10px;margin-left: 5px; border-left: 1px solid #999;width: 1px;height: 20px;";
    labelLine.setAttribute("id", "label-line-" + i);

    labelCopy.style.cssText =
      "position:relative;font-family:sans-serif;font-size:11px;color:#999";
    ...
    (function (i, t) {
      document.getElementById("label-box-" + i).addEventListener(
        "click",
        (e) => {
          if (!tl.paused()) tl.pause();
          tl.seek(t);
...
function setupFrames() {
  ...
  for (var i in timeline) {
    ...
    labelBox.style.cssText +=
      "left: " + (timeline[i] * magic - 16) + "px; top: 0px;";
    labelBox.setAttribute("id", "timeline-box-" + i);

    labelLine.style.cssText =
      "position: relative;left: 10px;margin-left: 5px; border-left: 1px solid black;width: 1px;height: 5px;";
    labelLine.setAttribute("id", "timeline-line-" + i);

    labelCopy.style.cssText =
      "position:relative;font-family:sans-serif;font-size:11px;color:#000";
    ...
    (function (i, t) {
      document.getElementById("timeline-box-" + i).addEventListener(
        "click",
        (e) => {
          if (!tl.paused()) tl.pause();
          tl.seek(t);
...
function checkClickTag() {
  if (tl.currentLabel() != theCurrentLabel) {
    ...
    document.getElementById("tl-clicktag").innerText =
      clickTags[tl.currentLabel()];
  }
}
...
function setupEverythingElse() {
  ...
  if (tl.duration() > 15) {
    tl.to(
      "#tl-progress",
      {duration: 0.1, backgroundColor: "#FF0000", ease: "sine.inOut"},
      15,
    );
  }
}
...
function durationCheck() {
  ...
  if (duration > 15) {
    console.log(
      "%c duration " + duration + "sec ",
      "background: #f52a0e; color: #fff; border-radius: 10px; padding:2px 5px;",
    );
  } else {
    console.log(
      "%c duration " + duration + "sec ",
      "background: #009900; color: #fff; border-radius: 10px; padding:2px 5px;",
    );
  }
}
...
function clickTagCheck() {
  if (window["clickTags"] == null) {
    console.log(
      `%c Error: clickTag object not found`,
      "background: #f52a0e; color: #fff; border-radius: 10px; padding:2px 5px;",
    );
    return null;
  }
  for (let i = 0; i < Object.keys(tl.labels).length; i++) {
    ...
    if (window[ct] == null) {
      ...
      console.log(
        `%c Error: ${ct} not defined in index.html `,
        "background: #f52a0e; color: #fff; border-radius: 10px; padding:2px 5px;",
      );
    }
  }
  ...
  background.addEventListener(
    ...
    (e) => {
      console.log(
        `%c clicked: ${tl.currentLabel()} → ${clickTags[tl.currentLabel()]} `,
        "background: #00a5e9; color: #fff; border-radius: 10px; padding:2px 5px;",
      );
    },
    false,
  );

  let clickTagObjects = Object.keys(window).filter((key) =>
    /^clickTag\d+$/.test(key),
  );
  let found;
  ...
  for (let i in clickTagObjects) {
    ...
    if (!found) {
      console.log(
        `%c Error: ${clickTagObjects[i]} not defined in logic.js `,
        "background: #f52a0e; color: #fff; border-radius: 10px; padding:2px 5px;",
      );
      tl.to(
        ".border",
...
function onKeyDown(event) {
  ...
  switch (event.which) {
    case 13:
      tl.restart().timeScale(1).repeat(0);
      break; // restart and reset
    case 32:
      tl.paused() ? tl.resume() : tl.pause();
      break; // spacebar: play/pause
    case 37:
      tl.pause().seek(tl.previousLabel());
      checkClickTag();
      break; // left arrow: skip to previous label
    case 39:
      tl.progress() == 1
        ? tl.seek(0).seek(tl.nextLabel())
        : tl.pause().seek(tl.nextLabel());
      checkClickTag();
      break; // right arrow: skip to next label
    case 38:
      tl.play();
      break; // Down arrow: Play reverse
    case 40:
      tl.reverse();
      break; // Up arrow: Play forward
    case 34:
      tl.timeScale(tl.timeScale() * 0.5);
      break; // PageUp: speed up
    case 33:
      tl.timeScale(tl.timeScale() * 2);
      break; // PageDn: slow down
    case 48:
      tl.timeScale(1);
      break; // 0 key: set speed to normal
    case 82:
      tl.repeat(-1);
      break; // R key: repeat
    case 67:
